...
(function ($, undefined) {
    ...
    var Grid = Widget.extend({
        ...
        saveRow: function () {
            ...
            if (container && editable && editable.end() && !that.trigger(SAVE, { container: container, model: model })) {
        ...
        _toolbar: function () {
            ...
            if (toolbar) {
                ...
                if (editable && editable.create !== false) {
                    container.on("touchend" + NS + " " + CLICK + NS, ".k-grid-add", function (e) {
                        e.preventDefault();
                        that.addRow();
                    })
                        .on("touchend" + NS + " " + CLICK + NS, ".k-grid-cancel-changes", function (e) {
                            e.preventDefault();
                            that.cancelChanges();
                        })
                        .on("touchend" + NS + " " + CLICK + NS, ".k-grid-save-changes", function (e) {
                            e.preventDefault();
                            that.saveChanges();
                        });
                }
            ...
            if (selectable) {
                ...
                if (that.options.navigatable) {
                    that.table.on("keydown" + NS, function (e) {
                        ...
                        if (e.keyCode === keys.SPACEBAR && e.target == that.table[0] && !current.is(".k-edit-cell,.k-header") &&
        ...
        _templates: function () {
            ...
            if ((!isEmptyObject(aggregates) && !footer.length) ||
                grep(that.columns,function (column) {
        ...
        _groupRowHtml: function (group, colspan, level) {
            var that = this,
                ...
                length,
                field = group.field,
                column = grep(that.columns, function (column) {
